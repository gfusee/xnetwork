...
def replace_file():
    ...
    for validator_dir in validator_dirs:
        external_toml_path = os.path.join(cwd, validator_dir, 'config', 'systemSmartContractsConfig.toml')
        subprocess.run(f"rm {external_toml_path}", shell=True)
        subprocess.run(f"cp systemSmartContractsConfig.toml {external_toml_path}", shell=True)
